...
export default function TicketDates() {
  ...
  async function fetchTicketInfo(timePeriod) {
    try {
      ...
      responseData.data.sort(
        (a, b) => b.dateTimeUnixFormat - a.dateTimeUnixFormat,
      );
      return responseData;
    } catch (error) {
      console.error(error);
  ...
  function displayLoadingOrTickets() {
    if (gigObjects[0] == "Loading") {
      ...
      return <h1>{gigObjects[0]}</h1>;
    } else if (gigObjects.length == 0) {
      return <h1>No Shows Currently Booked!</h1>;
    } else {
      return (
        <>
  ...
  return (
    <>
      ...
      <div className="ticket-gigs">{displayLoadingOrTickets()}</div>
    </>
  );
}
